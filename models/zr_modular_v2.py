"""ZR MODULAR / R2 — millimetres, X operator-right, Y lens/front, Z up.

Detailed design-development assembly. Camera interfaces remain provisional.
Run: conda run -n cad-ai python models/zr_modular_v2.py
"""
...
class Parameters:
    ...
    frame_width: float = 186
    frame_depth: float = 70
    post_offset_x: float = 80
...
def tapped(a,part,position,axis,size,depth,locating=False):
    ...
    if locating:
        # Orthogonal pairs on the mounting face; 15 mm between opposing pins.
        axes=((1,0,0),(0,1,0)) if axis[2] else ((1,0,0),(0,0,1))
        if axis[0]: axes=((0,1,0),(0,0,1))
        for t in axes:
            for side in (-1,1):
                a=drill(a,1.55,shift(position,t,side*P.arri_pin_spacing/2),axis,3.5,.18)
    return a
...
def screw(name,start,axis=(0,0,1),diameter=4,length=10,head=7,hh=3,group='core',material='steel',explode=(0,0,0)):
    # start is the seating face; shaft runs opposite the head's outward axis.
...
def nato(length):
    # 21 mm camera-accessory dovetail, not a Picatinny profile.
...
def add_thread_helix(a,x,y,z,height,size):
    """Real helical recess for selected prominent female threads."""
...
def build_frame():
    ...
    base=union(base,arca,box(28,20,8,x=-17,y=39,z=-12,r=4,edge=.7),
               box(24,18,14,x=80,y=26,z=-16,r=5,edge=.7))

    for label,x,y in [('left',-80,21),('right',80,-21)]:
    ...
    top=tapped(top,'top_plate',(80,7,102),(0,0,-1),'3/8-16',12,True)
    ...
    for x in (-37,-17,17,37):
        for yy in (16.8,27.2):
            top=drill(top,1.55,(x,yy,108 if abs(x)==17 else 102),(0,0,-1),3.5,.18)
    for y in (-19,-6):
        top=tapped(top,'top_plate',(-93,y,96),(1,0,0),'1/4-20',9)
    for x in (-25,25):
        top=drill(top,1.3,(x,22,108),(0,0,-1),4,.1)
        part(f'top_NATO_stop_{x}',cylinder(1.2,3,(x,22,106)),'steel',explode=(0,0,36))

    # Two real helical female thread features; remaining threads have exact callouts.
    for x in (-51,51):
        log(f'Cutting 3/8-16 helical thread at top x={x}')
        top=add_thread_helix(top,x,22,89,14,'3/8-16')

    # Cold shoe mounting holes in free left shoulder.
    for y in (-23,-12): top=tapped(top,'top_plate',(-80,y,102),(0,0,-1),'M3',7)
    # Camera tie-down slot: length along X, with a separate underside head recess.
    base=base.cut(box(18,6.7,24,x=-17,z=-20,r=3.3))
    base=base.cut(box(23,12.5,4,x=-17,z=-17,r=6.1))
    base=drill(base,1.55,(-17,10,-2),(0,0,-1),8,.2)
    for x in (-48,-7):
        for y in (-16,16):
    ...
    for x in (-61,-36,-10):
        base=tapped(base,'baseplate',(x,35,-7),(0,-1,0),'1/4-20',8)
    for y in (-22,-7): base=tapped(base,'baseplate',(-93,y,-7),(1,0,0),'1/4-20',9)
    base=tapped(base,'baseplate',(-55,0,-2),(0,0,-1),'3/8-16',10,True)
    for x,y in [(-35,0),(-73,-25),(-48,-28),(-23,-28),(-73,25),(-48,28),(-23,28)]:
        base=tapped(base,'baseplate',(x,y,-2),(0,0,-1),'1/4-20',8)
    for x in (-25,-9): base=tapped(base,'baseplate',(x,43,-4),(0,0,-1),'M4',7)
    # QD insert and two retention screw bores face forward.
    base=drill(base,6.15,(80,35,-9),(0,-1,0),10,.3)
    for x in (71,89): base=tapped(base,'baseplate',(x,35,-9),(0,-1,0),'M2.5',7)
    # Underside magnetic tool recess away from camera support and main screw.
    ...
    part('camera_anti_twist_pin',cylinder(1.5,10,(-17,10,-8)),'steel',explode=(0,0,12),note='Provisional camera hole center and engagement.')
    return top,base

def build_accessories():
    log('Building shoe, side rail, swing clamp, QD and adapter support')
    # Actual cold-shoe foot channel, open at the rear.
    shoe=box(22,26,5.5,x=-80,y=-17,z=102,r=2,edge=.4)
    shoe=shoe.cut(box(18.6,28,2,x=-80,y=-19,z=103.5))
    shoe=shoe.cut(box(12.6,28,5,x=-80,y=-19,z=104.5))
    for y in (-23,-12):
        shoe=drill(shoe,1.65,(-80,y,103.5),(0,0,-1),2,.1)
        shoe=shoe.cut(cone(2.85,1.55,1.3,(-80,y,103.5),(0,0,-1)))
        fastener=union(cylinder(1.38,6,(-80,y,96.2)),cone(1.5,2.8,1.3,(-80,y,102.2)))
        socket=cq.Workplane('XY').polygon(6,2.3).extrude(1).val().translate((-80,y,102.8))
    ...
    shoe=drill(shoe,1.5,(-80,-5,107.5),(0,0,-1),4,0)
    part('cold_shoe',shoe,explode=(0,0,43))

    rail=union(collar(-80,21,22),collar(-80,21,70),
               box(7,12,56,x=-94,y=21,z=22,r=1),
               nato(54).rotate((0,0,0),(0,1,0),-90).translate((-97,21,50)))
    # Carrier joins collars through small bridges.
    for z in (22,70):
        rail=union(rail,box(8,10,8,x=-91,y=21,z=z,r=1))
        rail=rail.cut(box(15,.8,10,x=-92,y=21,z=z-1))
        rail=drill(rail,7.55,(-80,21,z-.1),(0,0,1),8.2,0)
        rail=drill(rail,1.55,(-89.5,31,z+4),(0,-1,0),9.6,0)
        rail=tapped(rail,'left_NATO_carrier',(-89.5,20.6,z+4),(0,-1,0),'M3',10)
        rail=drill(rail,2.8,(-89.5,31,z+4),(0,-1,0),4,0)
        screw(f'side_rail_clamp_{z}',(-89.5,27,z+4),(0,1,0),diameter=3,length=13,head=5.3,hh=2.8,explode=(-28,0,0))
    for z in (29,71):
        rail=drill(rail,1.3,(-103,21,z),(1,0,0),4,.1)
        part(f'side_NATO_stop_{z}',cylinder(1.2,3,(-104,21,z),(1,0,0)),'steel',explode=(-25,0,0))
    for z in (40,60): rail=tapped(rail,'left_NATO_carrier',(-103,21,z),(1,0,0),'1/4-20',6)
    part('left_NATO_carrier',rail,explode=(-22,0,0))

    mount=union(collar(-80,21,8),box(26,13,8,x=-88,y=6,z=8,r=3,edge=.4))
    mount=drill(mount,7.55,(-80,21,7.9),(0,0,1),8.2,0)
    for x in (-95,-81): mount=tapped(mount,'cable_mount',(x,5,16),(0,0,-1),'M4',7)
    mount=mount.cut(box(18,.8,10,x=-89,y=21,z=7))
    mount=drill(mount,1.55,(-89.5,31,12),(0,-1,0),9.6,0)
    mount=tapped(mount,'cable_mount',(-89.5,20.6,12),(0,-1,0),'M3',10)
    mount=drill(mount,2.8,(-89.5,31,12),(0,-1,0),4,0)
    screw('cable_mount_clamp',(-89.5,28,12),(0,1,0),diameter=3,length=14,head=5.3,hh=2.8,explode=(-30,0,0))
    part('cable_mount',mount,explode=(-23,0,0))
    pivot_base=box(27,15,2.2,x=-87,y=5,z=16,r=3,edge=.3)
    for x in (-95,-81): pivot_base=drill(pivot_base,2.15,(x,5,18.2),(0,0,-1),3,0)
    for x in (-95,-81): screw(f'cable_carrier_M4_{x}',(x,5,20),length=10,head=6.5,hh=2.8,explode=(-30,0,10))
    part('cable_pivot_plate',pivot_base,explode=(-25,0,4))
    # Swinging cable frame has a real through opening along cable direction X.
    frame=box(26,28,27,x=-83,y=-8,z=20,r=3,edge=.7)
    frame=frame.cut(box(30,20,16,x=-83,y=-8,z=24,r=2,edge=.5))
    foot=box(27,10,1.8,x=-87,y=5,z=18.2,r=2)
    frame=union(frame,foot)
    frame=drill(frame,2.05,(-95,5,23),(0,0,-1),5,0)
    frame=drill(frame,2.15,(-81,5,23),(0,0,-1),5,0)
    # Clearance relief for pivot/latch heads and captive attachment seats.
    for x in (-95,-81): frame=drill(frame,3.4,(x,5,23),(0,0,-1),3,0)
    frame=tapped(frame,'cable_swing_frame',(-83,-8,47),(0,0,-1),'M4',7)
    part('cable_swing_frame',frame,explode=(-36,-12,8))
    jaw=box(23,19,3,x=-83,y=-8,z=36,r=1.5,edge=.3)
    jaw=drill(jaw,1.55,(-83,-8,39),(0,0,-1),2,0)
    part('cable_moving_jaw',jaw,explode=(-36,-12,14))
    part('cable_lower_pad',box(23,19,1.1,x=-83,y=-8,z=24,r=1),'rubber',explode=(-36,-12,8))
    part('cable_upper_pad',box(23,19,1.1,x=-83,y=-8,z=34.9,r=1),'rubber',explode=(-36,-12,14))
    knob=union(cylinder(6.5,3.5,(-83,-8,47.5)),cylinder(1.88,8.5,(-83,-8,39)),cylinder(1.5,1.5,(-83,-8,37.5)))
    # Scallops create a tactile machined thumbwheel.
    for k in range(18):
        a=2*math.pi*k/18
        knob=knob.cut(cylinder(.5,4,(-83+6.5*math.cos(a),-8+6.5*math.sin(a),47.3)))
    part('cable_thumbwheel',knob,'steel',explode=(-36,-12,22))

    # QD assembly, dimensions provisional pending selected swivel.
    qd=union(cylinder(6,9,(80,35,-9),(0,-1,0)),
             orient_z(box(22,12,1,r=2,edge=.25),(80,35,-9),(0,1,0)))
    qd=drill(qd,4.75,(80,36,-9),(0,-1,0),11,.4)
    qd=qd.cut(cylinder(5.4,2,(80,30,-9),(0,-1,0)))
    for x in (71,89): qd=drill(qd,1.4,(x,36,-9),(0,-1,0),2,.1)
    ...
    for x in (71,89): screw(f'QD_retention_{x}',(x,36,-9),(0,1,0),diameter=2.5,length=6,head=4.5,hh=1.6,explode=(0,20,0))
    ...
    # Side lug bridge is adjustable; the actual camera-lug interface is unresolved.
    lock=union(collar(80,-21,69,7),box(11,12,5,x=74.5,y=-15,z=71,r=2))
    lock=drill(lock,7.55,(80,-21,68.9),(0,0,1),7.2,0)
    ...
    screw('M2_5_lug_fastener',(70.8,-15,76),diameter=2.5,length=6,head=4.5,hh=2,explode=(22,0,9))

def build_grip():
    log('Building optional molded silicone grip configuration')
    spine=union(collar(80,-21,12),collar(80,-21,57),
                box(12,12,53,x=95,y=-21,z=12,r=2,edge=.6))
    for z in (12,57):
        spine=union(spine,box(10,10,8,x=90,y=-21,z=z,r=1))
        spine=spine.cut(box(15,.8,10,x=90,y=-21,z=z-1))
        spine=drill(spine,7.55,(80,-21,z-.1),(0,0,1),8.2,0)
        spine=drill(spine,1.55,(89.5,-31,z+4),(0,1,0),9.6,0)
        spine=tapped(spine,'grip_carrier',(89.5,-20.6,z+4),(0,1,0),'M3',10)
        spine=drill(spine,2.8,(89.5,-31,z+4),(0,1,0),4,0)
        screw(f'grip_clamp_{z}',(89.5,-28,z+4),(0,-1,0),diameter=3,length=14,head=5.3,hh=2.8,group='grip',explode=(38,0,0))
    for z in (25,48): spine=tapped(spine,'grip_carrier',(101,-21,z),(-1,0,0),'M4',12)
    part('grip_carrier',spine,group='grip',explode=(34,0,0))
    wp=cq.Workplane('XY').workplane(offset=10).center(110,-21).ellipse(8,13)
    ...
    grip=grip.cut(box(12.2,12.2,56,x=95,y=-21,z=10,r=2))
    for z in (25,48):
        grip=drill(grip,2.1,(121,-21,z),(-1,0,0),28,.2)
        grip=drill(grip,4,(123,-21,z),(-1,0,0),5,0)
        screw(f'grip_retention_{z}',(118,-21,z),(1,0,0),length=22,head=7,hh=3,group='grip',explode=(45,0,0))
...
def build_camera():
    ...
    # Explicit provisional socket/pin holes are documented, not measured camera data.
    body=drill(body,3.3,(-17,0,0),(0,0,1),7,0)
    body=drill(body,1.6,(-17,10,0),(0,0,1),3,0)
    # Rear display and underside door recesses.
    body=body.cut(orient_z(box(102,64,4,r=3),(-9,-22,39),(0,-1,0)))
    body=body.cut(box(51,39,1.2,x=40,y=-1,z=-.1,r=3))
    # Port locations are illustrative camera-proxy parameters, not measured data.
    ...
    # Screen and door motion views are separate reference states.
...
def export_all():
    ...
    # Check camera interference without intended tie-down/pin insertion.
    ...
    report={'revision':'R2','status':'DESIGN DEVELOPMENT — CAMERA FIT UNMEASURED',
            ...
            'notes':NOTES}
    # Pairwise broad-phase screening followed by exact BRep intersections.
    expected_threads={frozenset(pair) for pair in [
        ...
        ('baseplate','QD_retention_71'),('baseplate','QD_retention_89'),
        ('baseplate','lens_slider_-25'),('baseplate','lens_slider_-9'),
        ('cable_mount','cable_carrier_M4_-95'),('cable_mount','cable_carrier_M4_-81'),
    ...
    stationary=[p for p in main if p['name'] not in swinging | {'cable_carrier_M4_-81'}]
    for deg in range(0,91,15):
        for p in [p for p in main if p['name'] in swinging]:
            hits=intersecting(p['shape'].rotate((-95,5,0),(-95,5,1),-deg),stationary)
